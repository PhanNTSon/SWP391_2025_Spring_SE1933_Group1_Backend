...
public class RequestService {
    ...
    public void approvePublisher(Long requestID) {
        Request request = requestRepo.findById(requestID)
                .orElseThrow(() -> new RuntimeException(REQUEST_NOT_FOUND_MESSAGE));
        PublisherApplyRequest publisherApplyRequest = publisherApplyRequestRepo.findById(requestID)
                .orElseThrow(() -> new RuntimeException("PublisherApplyRequest not found"));
    ...
    public void approveFeedback(Long requestId, String response) {
        Request request = requestRepo.findById(requestId)
                .orElseThrow(() -> new RuntimeException(REQUEST_NOT_FOUND_MESSAGE));
        Feedback feedback = feedbackRepo.findById(requestId)
    ...
    public Page<AddingGameRequestDTO> getAllAddingGameRequest(Pageable pageable) {
        ...
        return addingGameRequestPage.map(addingGameRequest -> {
            ...
            addingGameRequestDTO
                    .setPublisherName(addingGameRequest.getRequest().getUser().getPublisher().getPublisherName());
            addingGameRequestDTO.setSendDate(addingGameRequest.getRequest().getTimeCreated().toString());
            addingGameRequestDTO.setPublisherId(
                    addingGameRequest.getRequest().getUser().getPublisher().getPublisherId().toString());
            return addingGameRequestDTO;
        });
    }

    public Page<PublisherApplyRequestDTO> getAllPublisherApplyRequest(Pageable pageable) {
        Page<PublisherApplyRequest> publisherApplyRequestPage = publisherApplyRequestRepo
                .findAllByRequestStateZero(pageable);
        return publisherApplyRequestPage.map(publisherApplyRequest -> {
            modelMapper.getConfiguration().setMatchingStrategy(MatchingStrategies.STRICT);
            PublisherApplyRequestDTO publisherApplyRequestDTO = modelMapper.map(publisherApplyRequest,
                    PublisherApplyRequestDTO.class);
    ...
    public Page<FeedbackDTO> getAllFeedback(Pageable pageable) {
        ...
        return feedbackPage.map(feedback -> {
            ...
            feedbackDTO.setUserId(
                    Long.parseLong(feedbackRepo.findUserIdByRequestId(feedback.getRequest().getRequestId())));
    ...
    public PublisherApplyRequestDTO getUserPublisherApplyDetails(Long userId) {
        PublisherApplyRequest publisherApplyRequest = publisherApplyRequestRepo
                .findByUserIdAndRequestStateZeroAndTwo(userId);
